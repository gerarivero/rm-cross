...
import { getAlumnoDetalle, getPlanes, getPromocionesActivas, getTurnos } from "./data";
...
export default async function AlumnoDetallePage({ params }: { params: { id: string } }) {
  const [alumno, planes, turnos, promociones] = await Promise.all([
  ...
  return (
    <>
      ...
      <main className="md:ml-20 min-h-screen flex flex-col">
        <AlumnoDetalleView alumno={alumno} planes={planes} turnos={turnos} promociones={promociones} />
      </main>
    </>
  );
}
